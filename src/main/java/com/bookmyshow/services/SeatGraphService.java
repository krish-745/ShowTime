...
public class SeatGraphService {
     ...
     */
    public List<Seat> findAdjacentSeats(int showId, int numSeats) {
        if (!seatGraph.containsKey((long) showId)) {
            buildSeatGraph(showId);
        }

        List<Seat> availableSeats = seatGraph.get((long) showId);
        Set<Seat> visited = new HashSet<>();

        for (Seat seat : availableSeats) {
            if (!visited.contains(seat)) {
                List<Seat> group = bfsFindAdjacent(seat, numSeats, visited, showId);
                if (!group.isEmpty()) {
                    return group;
                }
            }
    ...
    /**
     * BFS to find adjacent available seats.
     */
    private List<Seat> bfsFindAdjacent(Seat start, int numSeats, Set<Seat> visited, int showId) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (Seat neighbor : seatGraph.get((long) showId)) {
